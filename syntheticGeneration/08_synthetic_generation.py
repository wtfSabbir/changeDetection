...
def find_safe_edit_region(mask_path, box_size=256):
    """Finds a square region in the bottom half of the image that is 100% visible."""
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    h, w = mask.shape
    
    # Search randomly in the bottom half (the ground/road)
    for _ in range(100): 
        y = np.random.randint(h // 2, h - box_size) 
